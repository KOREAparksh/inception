...
function zsh() {
  ...
  sh -c "$(curl -fsSL https://raw.github.com/ohmyzsh/ohmyzsh/master/tools/install.sh)" && git clone https://github.com/zsh-users/zsh-autosuggestions ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-autosuggestions && sudo sed -i "s/plugins=(git)/plugins=(git zsh-autosuggestions)/g" ~/.zshrc && source ~/.zshrc
  echo "=================================================ZSH END"
}
